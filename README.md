# UNINats
A unidrop package for handling events with nats jetstream
...
func main() {
	...
	if err := client.CreateStream(ctx, jetstream.StreamConfig{
		...
		log.Println("🚨 [UNINATS]: Failed to initialize event bus:", err)
		os.Exit(1)
	}
...
```text
UNINATS.CLIENT=Nexor
UNINATS.DEBUG=false
UNINATS.URL=nats://localhost:4222
UNINATS.MAX_CONNECTIONS=5
UNINATS.MAX_RECONNECT_WAIT=5
```