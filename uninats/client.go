// Package uninats  provides a NATS client implementation with support for JetStream.
...
// Uninats represents a NATS client with JetStream support.
type uniNats struct {
...
func (n *uniNats) CreateStream(ctx context.Context, config jetstream.StreamConfig) error {
...
func (n *uniNats) GetEngine() *uniNats {
	return n
}

func (n *uniNats) Connect() {
...
func getConfig() *nexorConfig {
	var debugMode = false
	var clientName = "Uninats"
	var maxConn, maxWait = 5, 5
	if debugModeValue, found := os.LookupEnv("UNINATS.DEBUG"); found {
		debugMode = debugModeValue == "true"
	}

	if clientNameValue, found := os.LookupEnv("UNINATS.CLIENT"); found {
		clientName = clientNameValue
	}

	if maxConnValue := os.Getenv("UNINATS.MAX_CONNECTIONS"); maxConnValue != "" {
		maxConn, _ = strconv.Atoi(maxConnValue)
	}

	if maxWaitValue := os.Getenv("UNINATS.MAX_RECONNECT_WAIT"); maxWaitValue != "" {
...
// New creates a new Uninats instance connected to the specified NATS server.
...
// Returns a configured Uninats instance and any error encountered during connection.
func New(url string, opts ...nats.Option) Client {
	...
	return &uniNats{cfg: cfg}
}

// Close safely closes the NATS connection.
func (n *uniNats) Close() {
...
func (n *uniNats) JetStream() jetstream.JetStream {
	return n.js
}
