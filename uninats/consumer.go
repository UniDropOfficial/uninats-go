// Package uninats provides a NATS client implementation with support for protobuf message handling.
...
func (n *uniNats) Subscribe(
	...
	_, err = consumer.Consume(func(m jetstream.Msg) {
		...
		if err := proto.Unmarshal(m.Data(), msg); err != nil {
			if n.cfg.Debug {
				log.Printf("🚨 uninats: failed to decode protobuf: %v", err)
			}
		...
		if err := handler(ctx, msg, m); err != nil {
			if n.cfg.Debug {
				log.Printf("🚨 uninats: handler error: %v", err)
			}
	...
	if err != nil {
		if n.cfg.Debug {
			log.Printf("❌ uninats: failed to subscribe to subject: %s: %v", subject, err)
		}
		return err
	}

	if n.cfg.Debug {
		log.Printf("🚀 uninats: successfully subscribed to subject: %s", subject)
	}
