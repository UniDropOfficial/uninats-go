...
func (n *uniNats) Request(ctx context.Context, subject string, req proto.Message, factory func() proto.Message, timeout time.Duration) (proto.Message, error) {
	data, err := proto.Marshal(req)
	if err != nil {
		if n.cfg.Debug {
			log.Printf("❌ uninats: failed to marshal request: %v", err)
		}
		return nil, err
	}
	...
	if err != nil {
		if n.cfg.Debug {
			log.Printf("❌ uninats: request error: %v", err)
		}
		return nil, err
	}
	...
	if err := proto.Unmarshal(msg.Data, reply); err != nil {
		if n.cfg.Debug {
			log.Printf("❌ uninats: failed to unmarshal response: %v", err)
		}
		return nil, err
	}
