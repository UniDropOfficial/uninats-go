...
func (n *uniNats) Reply(subject string, reqFactory func() proto.Message, handler func(context.Context, proto.Message) (proto.Message, error)) error {
	_, err := n.conn.Subscribe(subject, func(m *nats.Msg) {
		req := reqFactory()
		if err := proto.Unmarshal(m.Data, req); err != nil {
			if n.cfg.Debug {
				log.Printf("❌ uninats: failed to unmarshal request: %v", err)
			}
			return
		}
		...
		if err != nil {
			if n.cfg.Debug {
				log.Printf("❌ uninats: request handler failed: %v", err)
			}
		...
		if err != nil {
			if n.cfg.Debug {
				log.Printf("❌ uninats: failed to marshal response: %v", err)
			}
			return
		}
	...
	if err != nil && n.cfg.Debug {
		log.Printf("❌ uninats: failed to subscribe for reply on %s: %v", subject, err)
	}
